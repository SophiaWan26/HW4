...
// insert book data 
app.post("/book",authorize,async (req, res:BookResponse )=>{
...
// insert author data
app.post("/author",authorize, async (req,res:AuthorResponse )=>{
    let authors = await db.all("SELECT * FROM authors");
...
app.post("/updateBook",authorize,async (req, res:BookResponse)=>{
    let books = await db.all("SELECT * FROM books");
    let authors = await db.all("SELECT * FROM authors");
...
app.delete("/book/", authorize, async (req,res:BookResponse)=>{
    if (!req.query.id) {
...
app.put("/updateBook",authorize,async (req, res:BookResponse)=>{
    let books = await db.all("SELECT * FROM books");
    let authors = await db.all("SELECT * FROM authors");
    const sql = "UPDATE books set  author_id = ? , title = ? , pub_year = ? , genre = ? where id = ? "
    const {id,author_id,title,pub_year,genre}=req.body
