...
app.get("/genrebook", async (req, res) => {
    if (!req.query.genre) {
...
// insert book data 
app.post("/book", authorize, async (req, res) => {
...
// insert author data
app.post("/author", authorize, async (req, res) => {
    let authors = await db.all("SELECT * FROM authors");
...
app.post("/updateBook", authorize, async (req, res) => {
    let books = await db.all("SELECT * FROM books");
    let authors = await db.all("SELECT * FROM authors");
...
app.delete("/book/", authorize, async (req, res) => {
    if (!req.query.id) {
...
app.put("/updateBook", authorize, async (req, res) => {
    let books = await db.all("SELECT * FROM books");
    let authors = await db.all("SELECT * FROM authors");
    const sql = "UPDATE books set  author_id = ? , title = ? , pub_year = ? , genre = ? where id = ? ";
    const { id, author_id, title, pub_year, genre } = req.body;
